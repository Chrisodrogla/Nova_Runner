...
# Define the number of months to go back and forward
months_back = 7  # Change this value as needed for past months
months_forward = 4  # Change this value as needed for future months

# Generate URLs for past and future months
Listing_Urls = []
...
for i in range(-months_back, months_forward + 1):
    ...
    if i < 0:
        # Calculate relative day offsets from Date_basis for past months
        start_value = f"-{(base_date - start_date).days}"
        end_value = f"-{(base_date - end_date).days}"
    else:
        # Calculate relative day offsets from Date_basis for future months
        start_value = f"{(start_date - base_date).days}"
        end_value = f"{(end_date - base_date).days}"
...
for listing in Listing_Urls:
    ...
    PageView = driver.find_element("xpath", """//div[@class="_c899vt"]""").text.split("listings is ")[1].split(" than")[
        0] if "listings is " in driver.find_element("xpath",
                                                    """//div[@class="_c899vt"]""").text and "the same" not in driver.find_element(
        "xpath", """//div[@class="_c899vt"]""").text else "No Data"
...
output_file = 'Airbnb_Metrics.json'
